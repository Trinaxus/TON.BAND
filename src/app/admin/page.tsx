...
export default function AdminPage() {
  ...
  const handleDeleteImage = async (galleryName: string, imgUrl: string) => {
    ...
    try {
      ...
      if (imgUrl.includes("/WebDisk/uploads/")) {
        ...
        const tonbandUrl = imgUrl.replace("https://www.tubox.de/WebDisk/uploads/", "https://tonbandleipzig.de/tonband/uploads/");
      ...
      if (imgUrl.includes("/Partycrasher/uploads/")) {
        // Ersetze nur für die Anzeige
        const tonbandUrl = imgUrl.replace("https://www.tubox.de/Partycrasher/uploads/", "https://tonbandleipzig.de/tonband/uploads/");
      ...
      cleanUrl = imgUrl
        .replace("https://tonbandleipzig.de/tonband/uploads/", "")
        .replace("https://www.tonbandleipzig.de/tonband/uploads/", "");
  ...
  const handleTestUpload = async () => {
    ...
    try {
      // Minimale Daten senden
      const uploadUrl = "https://tonbandleipzig.de/tonband/upload.php";
      console.log(`Test-Upload zu: ${uploadUrl}`);
      
      // Direkte Anfrage mit minimalen Headern
      ...
      console.log(`Test-Upload Status: ${res.status} ${res.statusText}`);
      const responseText = await res.text();
      console.log(`Test-Upload Antwort: ${responseText}`);
      
      newResults.push(`Test-Upload Status: ${res.status}, Antwort: ${responseText.substring(0, 100)}...`);
    } catch (err) {
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
          ...
          xhr.onload = function() {
            ...
            if (xhr.status === 200) {
              try {
                ...
                resolve({
                  success: true,
                  url: `https://tonbandleipzig.de/tonband/uploads/${year}/${gallery}/${file.name}`,
                  error: 'Ungültiges JSON, aber Status 200'
                });
              }
            } else {
        ...
        // Warte auf das Ergebnis des Uploads
        try {
          const data = await uploadPromise;
          if (data.success) {
            newResults.push(`✅ Upload erfolgreich! ${isVideo ? 'Video' : 'Bild'}-URL: ${data.url}`);
          } else {
            newResults.push(`❌ Fehler: ${data.error || 'Unbekannter Fehler'}`);
          }
        } catch (uploadError) {
          console.error('Upload-Fehler:', uploadError);
          newResults.push(`❌ Fehler: ${uploadError instanceof Error ? uploadError.message : String(uploadError)}`);
        }
      } catch (err) {
  ...
  {/* Feedback-Bereich */}
                    ...
                    >
                      {thumb ? (
                        <div className={styles.imageMini}>
                          <img src={thumb} alt="mini" className={styles.imageMiniBild} />
                          {isVideoUrl(thumb) && (
      ...
      {editingGallery && (
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            <h2 className={styles.modalTitle}>Galerie bearbeiten</h2>
            <p className={styles.modalSubtitle}>{editingGallery}</p>
            ...
            <div className={styles.modalFormGroup}>
              ...
              <div className={styles.modalFormGroup}>
                ...
                <div className={styles.accessTypeOptions}>
                  {/* Öffentlich */}
                  <label className={`${styles.accessTypeOption} ${editingMeta.accessType === 'public' ? styles.accessTypeOptionPublic : ''}`}>
                    <input 
                      type="radio" 
                      name="accessType" 
                      value="public"
                    ...
                    />
                    <div>
                      <div className={styles.accessTypePublic} style={{display: 'inline-flex', alignItems: 'center', marginBottom: '4px'}}>
                      ...
                      <div className={styles.modalSubtitle} style={{margin: 0}}>Jeder kann diese Galerie sehen</div>
                    </div>
                  </label>
                  
                  {/* Passwortgeschützt */}
                  <label className={`${styles.accessTypeOption} ${editingMeta.accessType === 'password' ? styles.accessTypeOptionPassword : ''}`}>
                    <input 
                      type="radio" 
                      name="accessType" 
                      value="password"
                    ...
                    />
                    <div>
                      <div className={styles.accessTypePassword} style={{display: 'inline-flex', alignItems: 'center', marginBottom: '4px'}}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{marginRight: '4px'}}>
                          <rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect>
                          <path d="M7 11V7a5 5 0 0 1 10 0v4"></path>
                        </svg>
                      ...
                      <div className={styles.modalSubtitle} style={{margin: 0}}>Zugriff nur mit Passwort</div>
                    </div>
                  </label>
                  
                  {/* Nur für Admins */}
                  <label className={`${styles.accessTypeOption} ${editingMeta.accessType === 'internal' ? styles.accessTypeOptionInternal : ''}`}>
                    <input 
                      type="radio" 
                      name="accessType" 
                      value="internal"
                    ...
                    />
                    <div>
                      <div className={styles.accessTypeInternal} style={{display: 'inline-flex', alignItems: 'center', marginBottom: '4px'}}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{marginRight: '4px'}}>
                          <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path>
                          <circle cx="12" cy="7" r="4"></circle>
                        </svg>
                      ...
                      <div className={styles.modalSubtitle} style={{margin: 0}}>Nur für eingeloggte Admins sichtbar</div>
                    </div>
                  </label>

                  {/* Gesperrt */}
                  <label className={`${styles.accessTypeOption} ${editingMeta.accessType === 'locked' ? styles.accessTypeOptionLocked : ''}`}>
                    <input 
                      type="radio" 
                      name="accessType" 
                      value="locked"
                      checked={editingMeta.accessType === 'locked'}
                      onChange={() => setEditingMeta({...editingMeta, accessType: 'locked'})}
                    />
                    <div>
                      <div className={styles.accessTypeLocked} style={{display: 'inline-flex', alignItems: 'center', marginBottom: '4px'}}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{marginRight: '4px'}}>
                          <rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect>
                          <path d="M7 11V7a5 5 0 0 1 10 0v4"></path>
                          <line x1="12" y1="17" x2="12" y2="17"></line>
                      ...
                      <div className={styles.modalSubtitle} style={{margin: 0}}>Für niemanden sichtbar (Ordner/ Galerie komplett ausgeblendet)</div>
                    </div>
                  </label>
                </div>
                  ...
                  <button
                    onClick={() => {
                      ...
                      setSelectedVideoForThumbnail({
                        url: `https://tonbandleipzig.de/tonband/uploads/${editingGallery}/gallery_thumb.jpg`,
      ...
      {activeSettingsGallery && (
        ...
        >
          ...
          <button
            className={`${styles.popupButton} ${styles.popupDownloadButton}`}
            onClick={async (e) => {
              e.stopPropagation();
              try {
                const current = activeSettingsGallery;
                if (!current) return;
                const galleryObj = galleries.find(g => g.name === current);
                const imageUrls = galleryObj?.images || [];
                if (!imageUrls.length) {
                ...
                const res = await fetch('/api/download-gallery', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ galleryName: current, imageUrls })
                });
                ...
                const safeName = current.replace(/\//g, '_');
                a.href = url;
                ...
                setSuccessMsg(`Download gestartet: ${safeName}.zip`);
                setTimeout(() => setSuccessMsg(''), 4000);
              } catch (err: any) {
                console.error('Download-Fehler:', err);
                setError(err?.message || 'Fehler beim Herunterladen der Galerie');
                setTimeout(() => setError(''), 5000);
              } finally {
                setActiveSettingsGallery(null);
              }
