...
const FILE_OPERATIONS_API = process.env.NEXT_PUBLIC_FILE_OPERATIONS_API || "https://tonbandleipzig.de/tonband/api/file_operations.php";
...
export default function FolderAdminPage() {
  ...
  useEffect(() => {
    async function fetchTree() {
      setLoading(true);
      try {
        console.log("API URL:", FILE_OPERATIONS_API);
        console.log("Token:", FILE_OPERATIONS_TOKEN ? "Vorhanden" : "Nicht vorhanden");
        
        // Zeige ausführlichere Debugging-Informationen
        console.log("Vollständige API URL:", `${FILE_OPERATIONS_API}?action=list`);
        console.log("Verwende Header:", { 'X-API-TOKEN': FILE_OPERATIONS_TOKEN });
        ...
        try {
          ...
          response = await fetch(`${FILE_OPERATIONS_API}?action=list`, {
            method: 'POST',
            headers: {
          ...
          if (!response.ok) {
            ...
            let jsonErr = '';
            try {
              jsonErr = JSON.stringify(JSON.parse(text));
            } catch (e) {
              // Kein valides JSON, ignoriere
            }
            throw new Error(`HTTP ${response.status}: ${text} ${jsonErr}`);
          }
          ...
          console.error("API-Aufruf fehlgeschlagen (POST):", err);
          ...
          try {
            ...
            console.log("GET-Fallback Antwort erhalten:", response.status, response.statusText);
            
            if (!response.ok) {
              console.error("GET-Fallback Antwort nicht OK:", response.status, response.statusText);
              throw new Error(`HTTP-Fehler bei GET-Fallback: ${response.status}`);
            }
            
            // Versuche Text zu bekommen und als JSON zu parsen
            const responseText = await response.text();
            console.log("GET-Fallback Antworttext:", responseText.substring(0, 100) + (responseText.length > 100 ? '...' : ''));
            
            try {
              data = JSON.parse(responseText);
            } catch (jsonError) {
              console.error("JSON parse error bei GET-Fallback:", jsonError, "Response war:", responseText);
              throw new Error("Ungültiges JSON in der GET-Fallback Antwort");
            }
            
            if (data.success) {
              console.log("Ordnerstruktur geladen (GET fallback):", data.folders);
              setDebugInfo(data);
              setTree(data.folders);
            } else {
              console.error("Fehler beim Laden der Ordnerstruktur (GET fallback):", data.error);
              setError(`API-Fehler (GET fallback): ${data.error || 'Unbekannter Fehler'}`);
              setTree([]);
            }
          } catch (fallbackErr) {
            console.error("Auch GET-Fallback fehlgeschlagen:", fallbackErr);
            const errorMessage = fallbackErr instanceof Error ? fallbackErr : 'Unbekannter Fehler';
            setError(`API nicht erreichbar: ${errorMessage}. Bitte überprüfe die API-URL und Netzwerkverbindung.`);
            setTree([]);
          }
        }
  ...
  async function handleRenameFolder() {
    ...
    if (/[\\\/]/.test(renameFolderName)) {
      setError('Der neue Ordnername darf keine Schrägstriche oder Backslashes enthalten.');
      return;
    }
    setRenamingFolder(true);
    try {
      // Sicherstellen, dass der Pfad immer mit 'uploads/' beginnt
      let currentFolder = selectedNode.path;
      if (!currentFolder.startsWith('uploads/')) {
        currentFolder = `uploads/${currentFolder}`;
      }
      const folderName = currentFolder.split('/').pop() || '';
      const parentFolder = currentFolder.substring(0, currentFolder.length - folderName.length - 1);
      const newPath = parentFolder ? `${parentFolder}/${renameFolderName}` : renameFolderName;
      const apiUrl = FILE_OPERATIONS_API;
      const apiToken = FILE_OPERATIONS_TOKEN;
      const oldPath = currentFolder;
      // newPath ist bereits oben deklariert
      const requestBody = `old=${encodeURIComponent(oldPath)}&new=${encodeURIComponent(newPath)}`;
      console.log("[RenameFolder] Request Body:", requestBody);
      const response = await fetch(`${apiUrl}?action=rename_folder`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
          'X-API-TOKEN': apiToken
      ...
      if (data.success) {
        console.log("Ordner erfolgreich umbenannt:", data);
        setRenameFolderName("");
        ...
        setError(`Fehler beim Umbenennen des Ordners: ${data.error}`);
      }
    } catch (error) {
