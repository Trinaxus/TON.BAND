...
export default function AdminGalleriesPage() {
    ...
    async function loadData() {
      try {
        ...
        if (data.galleries && typeof data.galleries === 'object') {
          ...
          // Sortiere Galerienamen nach Jahr (neueste zuerst)
          const sortedNames = Object.keys(data.galleries).sort((a, b) => {
  ...
  // Hilfsfunktion zum Ordner-Umbenennen
  async function renameGalleryFolder(oldName: string, newName: string): Promise<boolean> {
    const apiUrl = process.env.NEXT_PUBLIC_FILE_OPERATIONS_API;
    const apiToken = process.env.NEXT_PUBLIC_FILE_OPERATIONS_TOKEN;
    const old = `uploads/${oldName}`;
    const new_ = `uploads/${newName}`;
    try {
      const res = await fetch(`${apiUrl}?action=rename_folder`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-API-TOKEN": apiToken || ""
        },
        body: JSON.stringify({ old, new: new_ }) // Korrekte Keys!
      });
      const data = await res.json();
      console.log("[DEBUG] Antwort von rename_folder:", data);
      return !!data.success;
    } catch (err) {
