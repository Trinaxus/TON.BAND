...
import "../globals.css";
import "./globals-gallery.css"; // Importiere die globalen Galerie-Styles
...
const getVideoThumbnail = (videoUrl: string, galleryFiles: string[]): string => {
  console.log('getVideoThumbnail für:', videoUrl);
  ...
  if (videoUrl.includes('/tonband/uploads/')) {
    try {
      ...
      if (pathParts.length >= 3) {
        ...
        const videoThumbPath = `https://tonbandleipzig.de/tonband/uploads/${year}/${gallery}/video_thumb/${videoName}_thumb.jpg`;
        console.log('Generierter direkter Thumbnail-Pfad:', videoThumbPath);
        return videoThumbPath;
      }
    } catch (error) {
      console.error('Fehler beim Extrahieren der URL-Teile:', error);
...
const getVideosGalleryThumbnail = (galleryName: string, images: string[] = []): string => {
  ...
  if (isVideoGallery(galleryName)) {
    ...
    // 1. Suche zuerst nach Bildern im thumb/-Ordner
    console.log('Suche nach Bildern im thumb/-Ordner für Galerie:', galleryName);
    
    // Suche nach Bildern, die im thumb/-Ordner liegen
    const thumbImages = images.filter(url => url.includes('/thumb/') && !isVideoUrl(url));
    console.log('Gefundene Bilder im thumb/-Ordner:', thumbImages);
    
    if (thumbImages.length > 0) {
      // Verwende das erste Bild aus dem thumb/-Ordner
      console.log('Verwende Bild aus dem thumb/-Ordner:', thumbImages[0]);
      return ensureTonbandUrl(thumbImages[0]);
    }
    
    // 2. Fallback: Verwende einen festen Pfad zum thumb/-Ordner
    const thumbPath = `https://tonbandleipzig.de/tonband/uploads/${year}/${gallery}/thumb/gallery_thumb.jpg`;
    console.log('Kein Bild im thumb/-Ordner gefunden, verwende festen Pfad:', thumbPath);
    
    // 3. Fallback zum alten Pfad, falls das Thumbnail nicht im thumb/-Ordner existiert
    const legacyPath = `https://tonbandleipzig.de/tonband/uploads/${year}/${gallery}/gallery_thumb.jpg`;
    
    // Wir geben den Pfad zum thumb/-Ordner zurück und implementieren den Fallback im onError-Handler
    return thumbPath;
  }
      ...
      try {
        ...
        if (data.galleries && Object.keys(data.galleries).length > 0) {
          ...
          galleryData = {
            "2025/12.04.2025 - TonBand Geburtstags Session": [
              "https://tonbandleipzig.de/tonband/uploads/2025/12.04.2025%20-%20TonBand%20Geburtstags%20Session/img_6806057ece9462.57891697.jpg",
              "https://tonbandleipzig.de/tonband/uploads/2025/12.04.2025%20-%20TonBand%20Geburtstags%20Session/img_6806057ece9a72.92830942.jpg",
              "https://tonbandleipzig.de/tonband/uploads/2025/12.04.2025%20-%20TonBand%20Geburtstags%20Session/img_6806057ece9d45.42549713.jpg"
            ],
            "2025/02.05.2025 - TonBand Session": [
              "https://tonbandleipzig.de/tonband/uploads/2025/02.05.2025%20-%20TonBand%20Session/beispiel1.jpg"
            ],
            "2025/TON.BAND - VIDEOS": [
              "https://tonbandleipzig.de/tonband/uploads/2025/TON.BAND%20-%20VIDEOS/beispiel_video.mp4"
            ],
            "2024/21.12.2024 - TonBand Session": [
              "https://tonbandleipzig.de/tonband/uploads/2024/21.12.2024%20-%20TonBand%20Session/beispiel1.jpg"
            ]
          };
        ...
        Object.entries(galleryData).forEach(([galleryName, urls]) => {
          if (isVideoGallery(galleryName)) {
            ...
            galleryData[galleryName] = urls.filter(url => !isThumbnailUrl(url));
            console.log(`Galerie ${galleryName} ist eine Video-Galerie. Zeige nur Videos an:`, galleryData[galleryName]);
          }
        });
...
{Array.isArray(metadata[galleryName]?.tags) && metadata[galleryName]?.tags.length > 0 && (
  <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 6, margin: '6px 0 0 0' }}>
                          ...
                          <>
                            <Image
                              src={(() => {
                                ...
                                if (imageUrl.includes('/tonband/uploads/')) {
                                  try {
                                    const urlPath = imageUrl.split('/tonband/uploads/')[1];
                                    const pathParts = urlPath.split('/');
