...
import { ensureTonbandUrl } from './mediaHelpers';
...
export async function login(email: string, password: string): Promise<{ success: boolean; user?: any }> {
  return await fetchApi<{ success: boolean; user?: any }>(API_ENDPOINTS.LOGIN, {
    method: 'POST',
 ...
 */
export async function checkAuth(): Promise<{ isLoggedIn: boolean; user?: any }> {
  try {
    return await fetchApi<{ isLoggedIn: boolean; user?: any }>(API_ENDPOINTS.AUTH_ME, {
      method: 'GET',
      credentials: 'include',
      headers: {
